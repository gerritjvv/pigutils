...
/**
 * 
 * Reads the proto2 format which is:<br/>
 * <len><proto-bts><len><proto-bts>.<br/>
 * This differs from the Base64 line separated format.
 * 
 */
public class LZOProtoReader extends RecordReader<Writable, BytesWritable> {
	...
	final BytesWritable value = new BytesWritable(bts);
	...
	public void initialize(InputSplit split, TaskAttemptContext ctx)
		...
		if (codec == null) {
			throw new IOException("No codec for file "
					+ path
					+ " found, cannot run codecs are: "
					+ Arrays.toString(compressionCodecs.getCodecClasses(
							ctx.getConfiguration()).toArray()));
		}
