...
public abstract class LzoInputFormat<K, V> extends FileInputFormat<K, V> {
	private static final Logger LOG = LoggerFactory
			.getLogger(LzoInputFormat.class);
	...
	private final PathFilter visibleLzoFilter = new PathFilter() {
		...
		public boolean accept(Path path) {
			String name = path.getName();
			return !name.startsWith(".") && !name.startsWith("_")
					&& (name.endsWith(".lzo") || name.endsWith(".lzo_deflate"));
		}
	};
	...
	protected List<FileStatus> listStatus(JobContext job) throws IOException {
		...
		boolean recursive = job.getConfiguration().getBoolean(
				"mapred.input.dir.recursive", false);
		Iterator<FileStatus> it = files.iterator();
		while (it.hasNext()) {
			FileStatus fileStatus = it.next();
			FileSystem fs = fileStatus.getPath().getFileSystem(
	...
	public List<InputSplit> getSplits(JobContext job) throws IOException {
		...
		for (InputSplit genericSplit : defaultSplits) {
			...
			if (file.equals(prevFile)) {
				...
				index = LzoIndex.readIndex(
						file.getFileSystem(job.getConfiguration()), file);
				prevFile = file;
				prevIndex = index;
			}
			...
			long lzoEnd = index.alignSliceEndToIndex(
					end,
			...
			if (lzoStart != LzoIndex.NOT_FOUND && lzoEnd != LzoIndex.NOT_FOUND) {
				...
				LOG.debug("Added LZO split for " + file + "[start=" + lzoStart
						+ ", length=" + (lzoEnd - lzoStart) + "]");
			}
