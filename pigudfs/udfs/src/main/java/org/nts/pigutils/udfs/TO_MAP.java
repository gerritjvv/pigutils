...
/**
 * 
 * To Map converts a Tuple or a Bag of Tuples to a Map Structure. This makes it
 * easy to work with map structures<br/>
 * <p/>
 ...
 * Three or more, the first is taken as the key and the rest placed as a Tuple
 * and taken as the value<br/>
 * 
 * <p/>
...
public class TO_MAP extends EvalFunc<Map<String, Object>> {
	...
	public Map<String, Object> exec(Tuple input) throws IOException {
		...
		// if we have a database and the size is 1 then accept that we shoud
		// unroll this into a map.
		if (obj instanceof DataBag && input.size() == 1) {
			return convertBag((DataBag) obj);
		} else if (obj instanceof Map) {
//			Map<String, Object> hashMap = new HashMap<String, Object>();
//			conj(hashMap, (Map)obj);
			return (Map)obj;
		} else {
			Map<String, Object> hashMap = new HashMap<String, Object>();
	...
	private void conj(Map<String, Object> hashMap, Map<String, Object> map2) {
	...
	private void addTuple(Map<String, Object> hashMap, Tuple tuple)
		...
		if (tuple.get(0) instanceof DataBag) {
			throw new ExecException(
					"For an input with size > 1 the first entry cannot be a Bag");
		}

		if (tuple.size() == 1) {
			if (tuple.get(0) instanceof Map)
				conj(hashMap, ((Map) tuple.get(0)));
			else
				hashMap.put(tuple.get(0).toString().intern(), null);
		} else if (tuple.size() == 2) {
			if (tuple.get(0) instanceof Map)
				conj(hashMap, ((Map) tuple.get(0)));
			else
		     	hashMap.put(tuple.get(0).toString().intern(), tuple.get(1));
		} else {
			int size = tuple.size();
			...
			hashMap.put(tuple.get(0).toString().intern(), innerTuple);
		}
